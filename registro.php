			...
			<div class="row">
				<div class="col-md-12">
						<div class="" style="border-radius: 10px;margin-bottom: 40px;">
							<form autocomplete="off" action="<?php echo esc_url( site_url() . '/' . $GLOBALS['king_login'] . '?loginto=' . add_query_arg( array(),$wp->request ) ); ?>" id="login-form" method="post">
...
<script type="text/javascript">
	...
	jQuery("#voucher").blur(function(e){
		...
		const voucher = jQuery('#voucher').val().toLowerCase();
	...
	jQuery("#cpf").keyup(function(e){
		...
		if(tamanho == 14){
				jQuery.post("<?php print admin_url( 'admin-ajax.php' ); ?>", {action:'consulta_cpf',content:cpf}, function(response) {
		    	console.log('cpf request ---->',response);

		    if (response != "false") {
		    	//autorizado
		    	jQuery("#voucher").val('cpf-autorizado').hide();
		    	jQuery("#cpfID").val(response).hide();
		    }
		    // else{
		    // 	jQuery("#voucher").fadeIn();
		    // }
		    return;
			})
		}
